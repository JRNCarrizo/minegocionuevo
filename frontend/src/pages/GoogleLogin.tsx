...
export default function GoogleLogin() {
  ...
  useEffect(() => {
    ...
    if (subdominioParam && !isLoading && !autoLoginTriggered) {
    ...
    <div style={{
      ...
      <div style={{
          ...
          <p style={{
            ...
            {isLoading 
              ...
              : searchParams.get('subdominio') 
                ? `Continuar con Google para ${searchParams.get('subdominio')}` 
        ...
        <button
          ...
          disabled={isLoading || !searchParams.get('subdominio')}
          style={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            ...
            fontSize: '1rem',
            fontWeight: '600',
            cursor: isLoading || !searchParams.get('subdominio') ? 'not-allowed' : 'pointer',
            transition: 'all 0.2s ease',
            border: 'none',
            outline: 'none',
            width: '100%',
            opacity: isLoading || !searchParams.get('subdominio') ? 0.7 : 1,
            background: '#4285f4',
            color: 'white',
            boxShadow: '0 4px 12px rgba(66, 133, 244, 0.3)'
          }}
          onMouseOver={(e) => {
            if (!isLoading && searchParams.get('subdominio')) {
        ...
        >
          {isLoading ? (
            <>
              ...
              {autoLoginTriggered ? 'Redirigiendo a Google...' : 'Procesando...'}
            </>
          ) : (
            <>
              ...
              {autoLoginTriggered ? 'Iniciando automáticamente...' : 'Continuar con Google'}
            </>
          )}
        </button>
